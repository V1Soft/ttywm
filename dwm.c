...
enum { SchemeSel, SchemeClose, SchemePin, SchemeLast }; /* color schemes */
enum { NetSupported, NetWMName, NetWMState, NetWMFullscreen, NetActiveWindow, NetWMWindowType,
	   NetWMWindowTypeDialog, NetClientList, NetLast }; /* EWMH atoms */
enum { WMProtocols, WMDelete, WMState, WMTakeFocus, WMLast }; /* default atoms */
enum { ClkTagBar, ClkWinTitle, ClkTitleBar, ClkDateTime,
...
static int intersects(int px, int py, int x, int y, int w, int h);
...
static void toggle24hour(const Arg *arg);
static void togglefloating(const Arg *arg);
static void togglemonacle(const Arg *arg);
...
void buttonpress(XEvent *e) {
	...
	if (ev->window == selmon->barwin) {
		i = x = 0;
		if (intersects(ev->x, ev->y, 0, 0, TEXTW(selmon->sel->name), bh))
			click = ClkWinTitle;
		else if (ev->x > selmon->ww - TEXTW(buffer))
		  click = ClkDateTime;
		else
			click = ClkTitleBar;
	...
	if (ev->window == root) {
		...
		if (updategeom() || dirty) {
			...
			for (m = mons; m; m = m->next) {
				...
				XMoveResizeWindow(dpy, m->barwin, m->wx, m->by, m->ww, bh);//resizebarwin
			}
			focus(NULL);
			arrange(NULL);
		}
...
void drawbar(Monitor *m) {
  ...
  if (m == selmon) {
    x -= TEXTW(buffer);
    drw_text(drw, m->ww - TEXTW(buffer), 0, TEXTW(buffer), bh, buffer, 0);
  }
  if ((w = x) > bh) {
    x = 0;
    if (m->sel) {
...
int intersects(int px, int py, int x, int y, int w, int h) {
	if (px <= (w + x) && px >= x && py <= (h + y) && py >= y)
		return 1;
	return 0;
}
...
void manage(Window w, XWindowAttributes *wa) {
	...
	XSetWindowBorder(dpy, w, scheme[SchemeSel].border->pix);
...
void setup(void) {
	...
	/* init appearance */
	scheme[SchemeSel].border = drw_clr_create(drw, "#000000");
	scheme[SchemeSel].bg = drw_clr_create(drw, "#000000");
	scheme[SchemeSel].fg = drw_clr_create(drw, "#FFFFFF");
	/* init bars */
	updatebars();
	updatestatus();
...
void toggle24hour(const Arg *arg) {
  if (dtfmt)
    dtfmt = 0;
  else
    dtfmt = 1;
...
void togglemonacle(const Arg *arg) {
	if (!selmon->sel)
		return;
	if (!selmon->sel->isfullscreen) {
		...
		resizeclient(selmon->sel, selmon->sel->mon->mx, selmon->sel->mon->my+bh, selmon->sel->mon->mw, selmon->sel->mon->mh-bh);
...
void updatestatus(void) {drawbar(selmon);}
...
void updatetime(void) {
  ...
  if (!dtfmt)
    ...
    strftime(buffer, 26, "%I:%M %p", tm_info);
  drawbar(selmon);
}
